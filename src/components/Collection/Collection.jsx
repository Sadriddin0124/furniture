...
const Collection = ({title, subtitle, data}) => {
  ...
  return (
    <section className='w-[100%] flex flex-col items-center py-[30px] gap-[10px] md:gap-[20px]'>
      ...
      <div data-aos="fade-up" className='w-[100%] grid gap-[10px] sm:gap-[20px] grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 py-[20px] px-[10px] sm:px-[40px]'>
        {
            data?.map((item,index)=> {
                return <Link onClick={handleScrollToTop} to={`/single_page/${item?.id}`} key={index} className='flex flex-col gap-[10px] sm:gap-[20px] group'>
                    <img loading='lazy' src={item?.img} alt={item?.title} className='rounded-[20px] h-[150px] sm:h-[200px] object-cover'/>
