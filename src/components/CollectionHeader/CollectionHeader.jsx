...
const CollectionHeader = () => {
  ...
  //   Number(index) === 0 ? setItems(paginated1) : setItems(paginated2)
  // }
  // console.log(paginated1)
  const nextPage = () => {
  ...
  const items = activeCategory ? data?.filter(item=> item?.category === activeCategory) : paginated
  ...
  return (
    <header className="w-[100%] px-[20px] sm:px-[40px] py-[100px] md:flex-row flex-col-reverse flex gap-[30px] xl:gap-[50px]">
      <aside className="w-[100%] md:w-[300px] flex flex-col gap-[40px]">
        <form onSubmit={handleSearch} className="w-[100%] bg-gradient-to-r from-gray-200 to-gray-50 rounded-full p-[7px] flex justify-between items-center">
          <input
            type="text"
          ...
          />
          <button disabled={disabled} type="submit" className="w-[45px] h-[45px] bg-orange-300 rounded-full text-white text-[20px] flex items-center justify-center">
            <IoSearch />
          </button>
        </form>
        <div className="flex flex-col items-center gap-[40px]">
          <div className="flex justify-between w-[100%]">
            <h2>{title}</h2>
            <IoIosArrowForward onClick={()=>setCollectionStatus(!collectionStatus)} className={collectionStatus ? "rotate-[0deg] transition-all cursor-pointer" : "cursor-pointer rotate-[90deg] transition-all"}/>
          </div>
          <ul className={`${collectionStatus ? "h-[2px] overflow-hidden" : " overflow-visible h-[200px]"} transition-all flex flex-col gap-[10px] text-gray-500`}>
            {collections?.map((item, index) => (
              <li key={index} className=" cursor-pointer" onClick={()=>filterCategory(item)}>
                {item}
              </li>
            ))}
          </ul>
      ...
      <div className="flex flex-col gap-[40px]">
        <div className="w-[100%] flex justify-between flex-wrap gap-[20px] items-center">
          ...
          <select className="global__btn bg-black text-white" onChange={(e)=>handleSort(e.target.value)}>
        ...
        <div className="w-[100%] grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px] xl:gap-[40px]">
        ...
        <div className={`${activeCategory ? "hidden" : ""} w-[200px] flex justify-between gap-[10px] items-center`}>
          <button
